...
export class FeedManager {
    ...
    registerBuiltinSources() {
        ...
        this.register({
            ...
            fetch: async () => {
                try {
                    const res = await fetch('https://api.coingecko.com/api/v3/simple/price?ids=bitcoin,ethereum,solana,bnb&vs_currencies=usd&include_24hr_change=true', { signal: AbortSignal.timeout(8000) });
                    if (!res.ok)
                        return [];
        ...
        this.register({
            ...
            fetch: async () => {
                try {
                    const res = await fetch('https://min-api.cryptocompare.com/data/v2/news/?lang=EN&sortOrder=latest&limit=5', { signal: AbortSignal.timeout(8000) });
                    if (!res.ok)
                        return [];
                    const data = await res.json();
        ...
        this.register({
            ...
            fetch: async () => {
                try {
                    const res = await fetch('https://gamma-api.polymarket.com/markets?limit=5&order=volume&ascending=false&active=true', { signal: AbortSignal.timeout(8000) });
                    if (!res.ok)
                        return [];
                    const data = await res.json();
            ...
            fetch: async () => {
                ...
                try {
                    ...
                    return (data?.result?.transfers || [])
                        .filter((t) => parseFloat(t.value || '0') > 100)
                        .slice(0, 3)
                        .map((t) => ({
                        id: `whale-${t.hash}`,
        ...
        this.register({
            ...
            fetch: async () => {
                try {
                    const res = await fetch('https://open-api.coinglass.com/public/v2/funding?symbol=BTC', { signal: AbortSignal.timeout(8000) });
                    if (!res.ok)
                        return [];
                    const data = await res.json();
                    if (!data?.data)
                    ...
                    return [{
                            ...
                            category: 'signal',
                            metadata: { rates },
                            sentiment: rates.some((r) => r.fundingRate > 0.0005) ? 'bearish' : 'neutral',
                        }];
                }
                catch {
                    return [];
            ...
            fetch: async () => {
                try {
                    ...
                    const protos = (Array.isArray(data) ? data : [])
                        .filter((p) => p.tvl > 1_000_000 && p.change_1d !== null)
        ...
        this.register({
            ...
            fetch: async () => {
                ...
                try {
                    ...
                    const res = await fetch(`https://api.glassnode.com/v1/metrics/addresses/active_count?a=BTC&api_key=${key}&i=24h&limit=1`, { signal: AbortSignal.timeout(8000) });
                    if (!res.ok)
                        return [];
                    const data = await res.json();
        ...
        this.register({
            ...
            fetch: async () => {
                try {
                    const res = await fetch('https://min-api.cryptocompare.com/data/social/coin/latest?coinId=1182', // BTC
                    { signal: AbortSignal.timeout(8000) });
                    if (!res.ok)
                        return [];
                    const d = (await res.json())?.Data;
    ...
    getSources() { return Array.from(this.sources.keys()); }
}
