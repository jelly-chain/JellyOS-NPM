...
function displayCeremony(evm, sol, cos) {
    const R = '\x1b[91m';
    const G = '\x1b[92m';
    const Y = '\x1b[93m';
    const C = '\x1b[96m';
 ...
 */
export async function runVaultCeremony(jellyHome) {
    const addressFile = path.join(jellyHome, 'vault-addresses.json');
    if (fs.existsSync(addressFile)) {
        try {
            const existing = JSON.parse(fs.readFileSync(addressFile, 'utf-8'));
...
//# sourceMappingURL=vault-ceremony.js.map