...
export class PredictionModel {
    ...
    async calculateMomentum(prices) {
        if (prices.length < 14)
            return 0;
        const period = 14;
        const gains = [];
    ...
    async calculateVolatilityImpact(prices) {
        if (prices.length < 20)
            return 0;
        const returns = [];
    ...
    async analyzeVolume(volume, prices) {
        if (!volume || volume.length < 5)
            return 0;
    ...
    determineDirection(trend, momentum, sentiment) {
    ...
    calculateProbability(trend, momentum, volatility, volume, sentiment) {
        const raw = (trend * 0.3 + momentum * 0.2 + (1 - volatility) * 0.15 + volume * 0.15 + sentiment * 0.2);
        return Math.max(0.1, Math.min(0.95, (raw + 1) / 2));
    }
    calculateTargetPrice(prices, direction, probability) {
        const currentPrice = prices[prices.length - 1];
        const avgPrice = prices.reduce((s, p) => s + p, 0) / prices.length;
        const range = (Math.max(...prices) - Math.min(...prices)) / avgPrice;
        if (direction === 'up')
            return currentPrice * (1 + range * probability);
        if (direction === 'down')
            return currentPrice * (1 - range * probability);
        return currentPrice;
    }
    calculateConfidence(probability, volatility) {
    ...
    async predictWithMethod(input, method) {
        return this.predict(input);
    }
    async backtest(symbol, prices, horizon) {
        const results = [];
    ...
    resetCache() { this.predictionCache.clear(); }
}
