...
export const EVM_CHAINS = new Set<string>(['ethereum', 'bsc', 'arbitrum', 'base', 'polygon', 'avalanche', 'optimism', 'fantom', 'gnosis', 'scroll', 'linea', 'zksync', 'mantle', 'blast', 'celo']);

export function getNetworkId(chain: string): string | undefined {
  const c = CHAINS[chain];
  return c?.networkId ?? (EVM_CHAINS.has(chain) ? `${chain}-mainnet` : undefined);
}

export function getSymbol(chain: string): string {
