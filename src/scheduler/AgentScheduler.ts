...
function parseCron(expr: string): (now: Date) => boolean {
...
export class AgentScheduler {
  ...
  private load(): void {
    try {
      ...
      if (Array.isArray(raw)) this.tasks = raw as ScheduledTask[];
    } catch { /* start fresh */ }
  }
  ...
  private tick(onTrigger: (task: ScheduledTask) => void): void {
    const now    = new Date();
    ...
    for (const task of this.tasks) {
      ...
      if (shouldRun) {
        ...
        if (task.runOnce) task.enabled = false;
        this.save();
        onTrigger(task);
      }
    }
  ...
  private checkPriceTrigger(trigger: PriceTrigger): boolean {
    const tick = priceFeed.get(trigger.symbol.toLowerCase());
    if (!tick) return false;

    if (trigger.above !== undefined && tick.price >= trigger.above) return true;
    if (trigger.below !== undefined && tick.price <= trigger.below) return true;
    if (trigger.changePct !== undefined && Math.abs(tick.change24h) >= trigger.changePct) return true;
  ...
  readonly addTaskParams = Type.Object({
  ...
  async addTaskTool(_id: string, params: Static<typeof this.addTaskParams>) {
    let trigger: PriceTrigger | undefined;
    ...
    return {
      content: [{ type: "text" as const, text: `Scheduled: [${task.id}] ${task.name}${desc ? ` (${desc})` : ""}` }],
      details: { taskId: task.id, task },
    };
  }

  readonly listTasksParams = Type.Object({
    enabled_only: Type.Optional(Type.Boolean({ description: "Only show enabled tasks (default: false)" })),
  });

  async listTasksTool(_id: string, params: Static<typeof this.listTasksParams>) {
    const tasks = params.enabled_only ? this.tasks.filter(t => t.enabled) : this.tasks;

    if (tasks.length === 0) {
      return { content: [{ type: "text" as const, text: "No scheduled tasks." }], details: {} };
    }
  ...
  readonly removeTaskParams = Type.Object({
    id: Type.String({ description: "Task ID to remove" }),
  });

  async removeTaskTool(_id: string, params: Static<typeof this.removeTaskParams>) {
    const ok = this.removeTask(params.id);
    return {
      content: [{ type: "text" as const, text: ok ? `Task ${params.id} removed.` : `Task ${params.id} not found.` }],
