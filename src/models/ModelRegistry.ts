...
import type { ToolDef } from "../api/ExtensionAPI.js";
...
export function classifyModel(model: OpenRouterModel): ModelTier {
  for (const rule of TIER_RULES) {
    if (!rule.pattern.test(model.id)) continue;
    if (rule.notFree && parseFloat(model.pricing.prompt || "0") <= 0) continue;
    if (rule.minContext && (model.context_length || 0) < rule.minContext) continue;
    if (rule.minTokens && (model.top_provider.max_completion_tokens || 0) < rule.minTokens) continue;
    return rule.tier;
  }
  return "worker";
}
...
export class ModelRegistry {
  ...
  private tieredPool:    TieredPool = { orchestrator: [], analyst: [], worker: [], free: [] };
  private modelMap       = new Map<string, TieredModel>();
  ...
  async initialise(apiKey?: string): Promise<void> {
  ...
  private rebuildTiers(): void {
    ...
    for (const tier of (["orchestrator", "analyst", "worker", "free"] as ModelTier[])) {
  ...
  pick(tier: ModelTier): OpenRouterModel | null {
    ...
    for (const tm of pool) {
      ...
      if (!tm.available || (tm.failures >= 3 && now - tm.lastFailure < 300_000)) continue;
      return tm.model;
    }

    // Fallback: return any available model from any tier
    for (const t of (["worker", "analyst", "orchestrator", "free"] as ModelTier[])) {
      for (const tm of this.tieredPool[t]) {
        if (tm.available && tm.failures < 3) return tm.model;
      }
    }
   ...
   */
  private getTemperature(modelId: string, tier: ModelTier, envTemp: number): number {
    ...
    if (MODEL_TEMPS[modelId] !== undefined) return MODEL_TEMPS[modelId]!;
    // Any model with "thinking" in the ID needs 1.0
    if (/thinking/i.test(modelId)) return 1.0;
    // Tier defaults
    const TIER_TEMPS: Record<ModelTier, number> = {
    ...
    if (process.env.TEMPERATURE) return envTemp;
   ...
   */
  private getTokenBudget(modelId: string, tier: ModelTier, envMax: number): number {
  ...
  buildModelChain(userModels: string[]): ModelConfig[] {
    ...
    const tiers: ModelTier[] = ["orchestrator", "analyst", "analyst", "worker", "free"];
    let freeUsed = 0;

    for (const tier of tiers) {
      if (results.length >= 5) break;
      if (tier === "free" && freeUsed >= 2) continue;

      const picked = this.pick(tier);
      if (!picked) continue;
      // Skip if already in the list
      if (results.some(r => r.model === picked.id)) continue;

      const cfg = this.buildConfig(picked.id, tokens, temp);
      if (cfg) {
        results.push(cfg);
        if (tier === "free") freeUsed++;
      }
    }
  ...
  buildConfig(modelId: string, maxTokens: number, temperature: number, tier?: ModelTier): ModelConfig | null {
    ...
    if (modelId.startsWith("anthropic/") && env.ANTHROPIC_API_KEY) {
      ...
      const ANTHROPIC_API_ALIASES: Record<string, string> = {
        // Opus 4.x
  ...
  recordFailure(modelId: string): void {
    ...
    if (tm.failures >= 3) tm.available = false;
  }
  ...
  search(query: string, maxResults = 30): TieredModel[] {
    const q = query.toLowerCase();
    const results: TieredModel[] = [];
   ...
   */
  findCheapest(tier: ModelTier | "any", minContext = 0, maxCost = Infinity): TieredModel | null {
    const allTiers: ModelTier[] = ["orchestrator", "analyst", "worker", "free"];
    const pool = tier === "any"
  ...
  readonly listModelsParams = Type.Object({
  ...
  async listModelsTool(_id: string, params: Static<typeof this.listModelsParams>): Promise<{
    content: Array<{ type: "text"; text: string }>;
    details: Record<string, unknown>;
  }> {
    let results: TieredModel[];

    if (params.query) {
      results = this.search(params.query, params.limit ?? 20);
    } else if (params.tier && this.tieredPool[params.tier as ModelTier]) {
      results = this.tieredPool[params.tier as ModelTier];
    } else {
  ...
  readonly pickModelParams = Type.Object({
  ...
  async pickModelTool(_id: string, params: Static<typeof this.pickModelParams>): Promise<{
    content: Array<{ type: "text"; text: string }>;
    details: Record<string, unknown>;
  }> {
    const tier      = (params.tier ?? "any") as ModelTier;
  ...
  readonly summaryParams = Type.Object({});

  async summaryTool(): Promise<{
    content: Array<{ type: "text"; text: string }>;
    details: Record<string, unknown>;
  }> {
    return {
