...
// We lazily import node:sqlite to avoid crashing on Node <22
...
export class MemoryStore {
  ...
  save(sessionId: string, role: string, content: string, tags: string[] = []): void {
    ...
    try {
      ...
      this.db.prepare(
        `INSERT INTO messages (sessionId, role, content, tokens, ts, tags)
         VALUES (?, ?, ?, ?, ?, ?)`
      ).run(sessionId, role, capped, tokens, Date.now(), JSON.stringify(tags));
    } catch { /* best effort */ }
  }
  ...
  getRecentSessions(limit = 5): RecentSession[] {
    if (!this.available) return [];
    try {
      const rows = this.db.prepare(`
        SELECT
      ...
      return rows.map((r: Record<string, unknown>) => ({
        ...
        summary:   ((r["summary"] as string) ?? "").slice(0, 400),
  ...
  getSession(sessionId: string): MemoryEntry[] {
    ...
    try {
      const rows = this.db.prepare(
        `SELECT * FROM messages WHERE sessionId = ? ORDER BY ts ASC`
      ).all(sessionId);
      return rows.map((r: Record<string, unknown>) => ({
        ...(r as object),
        tags: JSON.parse((r["tags"] as string) ?? "[]"),
  ...
  getStats(): { totalMessages: number; totalSessions: number } {
    if (!this.available) return { totalMessages: 0, totalSessions: 0 };
    try {
      const { count }    = this.db.prepare(`SELECT COUNT(*) as count FROM messages`).get() as { count: number };
      const { sessions } = this.db.prepare(`SELECT COUNT(DISTINCT sessionId) as sessions FROM messages`).get() as { sessions: number };
      return { totalMessages: count, totalSessions: sessions };
    } catch { return { totalMessages: 0, totalSessions: 0 }; }
  }
}
