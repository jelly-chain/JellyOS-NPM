...
export class PriceFeed extends EventEmitter {
  ...
  private tracking:      Set<string> = new Set(["btc", "eth", "sol", "bnb", "matic", "arb", "op", "avax"]);
  ...
  track(...symbols: string[]): void {
    for (const s of symbols) this.tracking.add(s.toLowerCase());
  }
  ...
  async fetchAll(): Promise<void> {
    ...
    if (updated) this.emit("prices", [...this.prices.values()]);
  }

  private async fetchBinance(): Promise<PriceTick[]> {
  ...
  private async fetchCoinGecko(): Promise<PriceTick[]> {
    ...
    const res = await fetch(
      ...
      { signal: AbortSignal.timeout(10_000), headers: { "User-Agent": "JellyOS/1.0" } },
    );
  ...
  getMultiple(symbols: string[]): PriceTick[] {
    return symbols.map(s => this.get(s)).filter(Boolean) as PriceTick[];
  }
  ...
  getTopMovers(limit = 5): PriceTick[] {
    return [...this.prices.values()]
  ...
  tickerLine(maxSymbols = 5): string {
    return [...this.prices.values()]
      .slice(0, maxSymbols)
      .map(t => {
...
export async function getPricesTool(_id: string, params: Static<typeof getPricesParams>) {
  const ticks = priceFeed.getAll();
  if (ticks.length === 0) {
    return {
      content: [{ type: "text" as const, text: "No price data available yet. Please wait for the first price update." }],
      details: {},
    };
  }
  ...
  if (results.length === 0) {
    return {
      content: [{ type: "text" as const, text: `No data for: ${params.symbols.join(", ")}. Available: ${ticks.map(t => t.symbol).join(", ")}` }],
      details: {},
    };
  }
...
export async function topMoversTool(_id: string, params: Static<typeof topMoversParams>) {
  const movers = priceFeed.getTopMovers(params.limit ?? 5);
  const text   = movers.map(t => priceFeed.formatPrice(t)).join("\n");
  return {
    content: [{ type: "text" as const, text: text || "No data available" }],
...
export async function marketOverviewTool() {
  ...
  if (ticks.length === 0) {
    return {
      content: [{ type: "text" as const, text: "No market data available yet." }],
      details: {},
    };
  }
