...
export function macd(prices: number[], fast = 12, slow = 26, signal = 9): {
  ...
  for (let i = 0; i < fastEMA.length; i++) {
    ...
    if (slowIdx >= 0) macdLine.push(fastEMA[i]! - slowEMA[slowIdx]!);
  }
...
export function bollingerBands(prices: number[], period = 20, deviations = 2): {
  upper: number[];
  middle: number[];
  lower: number[];
  width: number[];
...
export function volumeProfile(volumes: number[], prices: number[], levels = 10): {
  priceRange: string;
  volume:     number;
  pct:        number;
}[] {
  ...
  const profile: { volume: number; low: number; high: number }[] = [];
...
export function detectCrossovers(fast: number[], slow: number[]): {
  type: "golden_cross" | "death_cross" | null;
  index: number;
} {
  if (fast.length < 2 || slow.length < 2) return { type: null, index: -1 };
  ...
  for (let i = fast.length - 1; i >= Math.max(0, fast.length - 5); i--) {
    ...
    if (prevFast <= prevSlow && currFast > currSlow) return { type: "golden_cross", index: i };
    if (prevFast >= prevSlow && currFast < currSlow) return { type: "death_cross", index: i };
  }
  return { type: null, index: -1 };
}

export function rsiSignal(rsiValues: number[]): AnalysisResult {
  const last = rsiValues[rsiValues.length - 1];
  if (last === undefined) return { indicator: "RSI", value: 0, signal: "neutral" };
  if (last > 70) return { indicator: "RSI", value: Math.round(last * 100) / 100, signal: "bearish", metadata: { condition: "overbought" } };
  if (last < 30) return { indicator: "RSI", value: Math.round(last * 100) / 100, signal: "bullish", metadata: { condition: "oversold" } };
  return { indicator: "RSI", value: Math.round(last * 100) / 100, signal: "neutral" };
}

export function bollingerSignal(price: number, bands: { upper: number[]; lower: number[] }): AnalysisResult {
  const upper = bands.upper[bands.upper.length - 1];
  const lower = bands.lower[bands.lower.length - 1];
  if (upper === undefined || lower === undefined) return { indicator: "Bollinger", value: 0, signal: "neutral" };
  const position = (price - lower) / (upper - lower);
  if (position > 0.95) return { indicator: "Bollinger", value: Math.round(position * 100), signal: "bearish", metadata: { condition: "upper band touch" } };
  if (position < 0.05) return { indicator: "Bollinger", value: Math.round(position * 100), signal: "bullish", metadata: { condition: "lower band touch" } };
  return { indicator: "Bollinger", value: Math.round(position * 100), signal: "neutral" };
}

export function macdSignal(macdData: { macd: number[]; signal: number[]; histogram: number[] }): AnalysisResult {
  const hist  = macdData.histogram;
  if (hist.length < 2) return { indicator: "MACD", value: 0, signal: "neutral" };
  const last  = hist[hist.length - 1]!;
  const prev  = hist[hist.length - 2]!;
  const value = Math.round(last * 1_000_000) / 1_000_000;
  if (last > 0 && prev <= 0) return { indicator: "MACD", value, signal: "bullish", metadata: { condition: "histogram crossed above zero" } };
  if (last < 0 && prev >= 0) return { indicator: "MACD", value, signal: "bearish", metadata: { condition: "histogram crossed below zero" } };
  if (last > prev)  return { indicator: "MACD", value, signal: "bullish", metadata: { condition: "histogram rising" } };
  if (last < prev)  return { indicator: "MACD", value, signal: "bearish", metadata: { condition: "histogram falling" } };
  return { indicator: "MACD", value, signal: "neutral" };
}

// ── Full analysis ────────────────────────────────────────────────────────────

export function fullAnalysis(ohlcv: OHLCV[]): AnalysisResult[] {
  const closes  = ohlcv.map(c => c.close);
  const highs   = ohlcv.map(c => c.high);
  const lows    = ohlcv.map(c => c.low);
  const volumes = ohlcv.map(c => c.volume);

  if (closes.length < 30) return [{ indicator: "ERROR", value: 0, signal: "neutral", metadata: { message: `Need 30+ candles, got ${closes.length}` } }];
...
export async function getCandlesTool(
  _id: string,
  params: Static<typeof getCandlesParams>,
) {
  ...
  if (!VALID_INTERVALS.has(interval)) {
    return {
      content: [{ type: "text" as const, text: `Invalid interval "${interval}". Valid: ${[...VALID_INTERVALS].join(", ")}` }],
      details: {},
    };
  }
  ...
  try {
    const res = await fetch(
      ...
      { signal: AbortSignal.timeout(12_000), headers: { "User-Agent": "JellyOS/1.0" } },
    );

    if (!res.ok) {
      return {
        content: [{ type: "text" as const, text: `Binance API error ${res.status} for ${symbol}. Check symbol.` }],
        details: {},
      };
    }
    ...
    const candles: OHLCV[] = raw.map(c => ({
      timestamp: c[0] as unknown as number,
    ...
    if (analyze && candles.length >= 30) {
      ...
      for (const r of results) {
        if (r.indicator === "SUMMARY") continue;
    ...
    return {
      content: [{ type: "text" as const, text: `Failed to fetch candles for ${symbol}: ${msg}` }],
      details: {},
    };
  }
}
